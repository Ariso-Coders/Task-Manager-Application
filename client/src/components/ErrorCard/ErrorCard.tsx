...
const ErrorCard: React.FC<ErrorCardProps> = (props) => {
    ...
    return (
        <div className="absolute top-0 left-0 w-full h-full flex items-center justify-center bg-black bg-opacity-50 z-1000  ">
            <div className=" py-5 px-10 bg-gray-900 flex flex-col justify-center items-center rounded-lg bg-white gap-10">
                ...
                <section className="mt-5vh flex gap-5">
                    {props.details.btn1[0] && (
                        <button id="ER_btn1" onClick={btnHandler1}  className="text-base px-3 py-1 border-none bg-green cursor-pointer rounded  hover:text-white hover:border-green-500"
                        >{props.details.btn1[1]} </button>
                    )}
                    {props.details.btn2[0] && (
                        <button id="ER_btn2"  onClick={btnHandler2}  className="text-base px-3 py-1 border-transparent bg-gray cursor-pointer rounded  hover:text-white hover:border-gray" >{props.details.btn2[1]}</button>
                    )}
                </section>
            </div>
        </div>
